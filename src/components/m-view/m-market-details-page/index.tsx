...
export default function MMarketDetailsPage({ apiData }: { apiData: any }) {
  ...
  // Is useEffect ki dependency array se matchOddsData nikaal di hai loop khatam karne ke liye


  useEffect(() => {
    if (!apiData?.matchOddsData || apiData.matchOddsData.length === 0) return;

    const marketIds = apiData.matchOddsData
      .map((m: any) => m?.exMarketId || m?.marketId)
      .filter((id: any) => !!id);
    ...
    const offOdds = webSocketService.onEvent<any>("odds", (raw) => {
      try {
        ...
        if (typeof raw === "string") payload = JSON.parse(raw);
          ...
          payload = typeof raw[1] === "string" ? JSON.parse(raw[1]) : raw[1];
        }
        ...
        setMatchOddsData((prev: any[]) => {
          ...
          return prev.map((m) => {
            ...
            const runners = (m.runners || []).map((r: any) => {
              const key = String(r.selectionId);
              const exEntry = payload.ex?.[key] || payload.ex?.[r.selectionId];
              if (!exEntry) return r;
              ...
              return {
                ...
                ex: {
                  ...r.ex,
                  availableToBack: mergeSide(r.ex?.availableToBack, exEntry.availableToBack),
                  availableToLay: mergeSide(r.ex?.availableToLay, exEntry.availableToLay),
                },
                _updateKey: Date.now(),
              };
            });

            return { ...m, runners, _updateKey: Date.now() };
          });
        });
  ...
  }, [eventId, apiData?.matchOddsData?.length]);
  ...
  // 🔥 Yeh effect matchOddsData ke updates ko filteredMarketData mein sync karega
  useEffect(() => {
    if (matchOddsData.length > 0 && activeCategory) {
      let filtered = [];
      if (activeCategory === "Popular") {
        filtered = matchOddsData.filter((m) => m.popular && m.status !== "CLOSED");
      } else if (activeCategory === "ALL" || activeCategory === "All Market") {
        filtered = [...matchOddsData];
      } else {
        filtered = matchOddsData.filter((m) => m.marketName === activeCategory);
      }
  ...
  const formatSize = (size: number) => {
    ...
    if (size >= 1000) {
      const val = size / 1000;
      // Agar point ke baad zero hai toh hatayein (e.g. 1.00K -> 1K)
                              ...
                              >
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px] md:font-normal">
                                    ...
                                    {formatSize(runner?.ex?.availableToBack[0]?.size || 0)}
                                  </span>
                                </div>
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px]">
                                    ...
                                    {formatSize(runner?.ex?.availableToBack[2]?.size || 0)}
                                  </span>
                                </div>
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px]">
                                    ...
                                    {formatSize(runner?.ex?.availableToBack[1]?.size || 0)}
                                  </span>
                                </div>
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px]">
                                    ...
                                    {formatSize(runner?.ex?.availableToLay[0]?.size || 0)}
                                  </span>
                                </div>
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px]">
                                    ...
                                    {formatSize(runner?.ex?.availableToLay[1]?.size || 0)}
                                  </span>
                                </div>
                                ...
                                >
                                  ...
                                  <span className="block text-xs lg:text-[10px]">
                                    ...
                                    {formatSize(runner?.ex?.availableToLay[2]?.size || 0)}
                                  </span>
                                </div>
                              </div>
                            </div>
